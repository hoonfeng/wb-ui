...
> 状态：✅ 已实现（2026-08-13）。P3.1 / P3.2 / P3.3 / P3.4（及追加的 P3.5）全部落地。
> 目标：让 CSS `mask-image` 达到 CSS Masking Level 1 的可用语义，并补齐 SVG `<mask>`。
>
> **阅读指引**：§一~§五 是**立项当时**的现状分析与方案（保留作设计依据，含未来时表述）；
> 真实落地情况与代码落点见 **§六 实现记录**。§一「1.3 缺失项」表里的 6 项现已全部补齐；
> 「1.1 已实现：仅遮 background + border」描述的是**改造前**的状态——该旧路径
> （`paintObjectBackground` 内只包 background/border 的 mask）已被移除。

## 一、现状（以代码为准）

### 1.1 已实现：仅遮 background + border
- 位置：`engine/rendering/renderpipeline.go` 的 `paintObjectBackground`（约 1553 行）。
...
### 1.2 skia 原语已就绪
- `engine/platform/graphics/canvas.go`：`SaveLayerForMask(rect)`（离屏层）+ `ApplyImageMask(img, rect)`（`BlendModeDstIn`，取图片 alpha，忽略 RGB）+ `Restore`。
- 图片加载：`engine/rendering/image.go` 的 `DecodedImage.SkiaImage()` + `loadBackgroundImage`。
...
- **涉及文件**：`engine/rendering/renderpipeline.go`（mask apply 处）、可能新增 `engine/rendering/mask.go`。
- **验证**：`mask-size: cover`/`contain`/`50%`、`mask-repeat: no-repeat` 各写一个 `engine/rendering/mask_test.go` 用例，对比半透明像素区域几何。
...
- **涉及文件**：`engine/rendering/renderpipeline.go`、`engine/rendering/image.go`。
...
- **涉及文件**：`engine/rendering/renderpipeline.go`（`walkSubtreeExcluded` + `paintObjectBackground`）、`engine/rendering/painter.go`。
...
5. 全量 `go test ./...` 无回归，`engine/rendering/mask_test.go` 扩充为参数化用例。

## 六、实现记录（2026-08-13，全部完成）

### P3.1 子树级遮罩（核心）
- `engine/rendering/renderlayer.go`：`RequiresLayer` 增加 mask-image 判断——mask 元素必须有
  自己的 RenderLayer，才能像 opacity 一样在 layer 层包裹整棵子树。
- `engine/rendering/renderpipeline.go`：新增 `paintLayerWithEffects`（mask 离屏层在最外、
...
- `engine/rendering/mask.go`：`applyMaskLayer` 复用 `computeBackgroundDest` 解析 size/position；
...
- `engine/platform/graphics/canvas.go`：`ApplyImageMaskMode` / `ApplyImageMaskTiled` 增加
...
### P3.4 SVG `<mask>`
- `engine/rendering/svg.go`：`svgMask` 结构 + `<defs>` 解析 `<mask>`（maskUnits/maskContentUnits/
...
- `engine/rendering/mask.go`：`mask-image: url(file.svg#maskId)` / `url(data:...#id)` 引用 SVG
  mask；`mask-type` 与 CSS `mask-mode` 交互（match-source 时 SVG 默认 luminance）。

### 测试
- `engine/rendering/mask_test.go`：TestMaskImageAlpha / TestMaskImageMasksDescendants /
  TestMaskSizeNoRepeat / TestMaskModeLuminance / TestMaskPropertyNoCrash。
- `engine/rendering/svg_mask_test.go`：TestSVGMaskParseAndRender / TestMaskImageSVGMask /
...
- `engine/rendering/svg.go`：把 `buildSVGDocument` 的 `case "mask"` 解析逻辑提取为独立
  `parseMaskElement(defEl *dom.Element) *svgMask`，供同文档引用复用。
- `engine/rendering/mask.go`：`applyMaskLayer` 增加 `ownerEl *dom.Element` 参数——当
...
- `engine/rendering/renderpipeline.go`：`paintLayerWithEffects` 从 `layer.Owner().Node()`

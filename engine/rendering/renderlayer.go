...
// will-change / containment; the same trigger set engine/layout/positioned.go uses via
// layout.CreatesContainingBlockForFixed).
//
// A fixed box trapped by such an ancestor belongs to that ancestor's
// containing-block chain: engine/layout/positioned.go already resolves its insets

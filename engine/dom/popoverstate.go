...
// 而把**算法**放在独立包 wb-ui/engine/popover —— 这样：
...
//     wb-ui/engine/popover，dom 本身从不改变这些字段）。
//
...
//
// 字段含义与规范槽一一对应，命名以可读性优先；所有字段由 wb-ui/engine/popover 包的
...
// Popover 返回元素的 popover 状态（可写指针）。调用方为 wb-ui/engine/popover 包的

...
type Element struct {
	...
	// 见 popoverstate.go；由 wb-ui/engine/popover 包维护，dom 包本身不改变它。
...
// el.onclick IDL attribute (see engine/js/bindings/lazyelement.go setOnClick). Not part of the
...
// :user-invalid (engine/css/selectorchecker.go); the engine sets it from its real user
...
// 时关闭它）。由 wb-ui/engine/popover 在 init 里注册——dom 包不依赖它。
...
// (engine/css/selectorchecker.go) and the ::backdrop box generation (layout/rendering),
// so all three agree.
...
// 失效），computedStyleFor 的结果缓存（engine/js/bindings/csscache.go）只按全局

// css/validity.go — 表单约束校验（constraint validation）的 CSS 侧接入。
//
// CSS 的 :valid / :invalid / :in-range / :out-of-range 需要知道元素的约束
// 校验状态，而这属于 HTML 表单语义（html5 包的 ValidityState）。css 包不能
// 直接 import html5——html5 已经 import css（html5/defaultcss.go 的 UA 表），
// 反向依赖会形成包循环。因此这里定义一组最简数据结构，由宿主在初始化时
// 注入实现（html5 包的 init 里注册，见 html5/constraint.go 的
